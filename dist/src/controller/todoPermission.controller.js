...
export class TodoPermissionController {
    ...
    updatePermission = async (req, res) => {
        try {
            ...
            const result = await this.todoPermissionService.updatePermission(paramsValidation.todoId, paramsValidation.userId, bodyValidation, req.user.userId);
            if (result.success) {
    ...
    revokePermission = async (req, res) => {
        try {
            ...
            const paramsValidation = permissionParamsSchema.parse(req.params);
            const result = await this.todoPermissionService.revokePermission(paramsValidation.todoId, paramsValidation.userId, req.user.userId);
            if (result.success) {
                res.status(200).json(result);
            }
            else {
